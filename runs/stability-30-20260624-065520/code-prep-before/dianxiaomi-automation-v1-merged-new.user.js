// ==UserScript==
// @name         DXM Automation V1 - NEW v1.1.36
...
(function () {
  ...
  let visibleEditRulesAutoApplied = false;
  ...
  function collectBrandCandidates(item) {
    ...
    const firstTitleToken = normalizeSpaces(titleSource).split(/\s+/)[0] || '';
  ...
  function hasForbiddenCommerceTerms(text) {
    const source = String(text || '').toLowerCase();
    return FORBIDDEN_COMMERCE_TERMS.some((term) => source.includes(term.toLowerCase()));
  }
  ...
  function buildCompliantProductTitle(originalTitle, item) {
    let title = stripForbiddenCommerceTerms(sanitizePlatformText(firstNonEmpty(originalTitle, item && item.title, fallbackProductTitle(item))), item)
    ...
    return sanitizePlatformText(title || fallbackProductTitle(item));
  }
  ...
  function buildPayloadFromEdit(editData) {
    ...
    if (!detailPlainText || detailPlainText.length < EDIT_PAGE_RULES.pcDescriptionMinChars || hasForbiddenCommerceTerms(detailPlainText)) {
      const compliantDescription = buildCompliantPcDescription(complianceSource, payload.subject);
      payload.detailWeb = textToDetailWeb(compliantDescription);
      payload.detailMobile = buildDetailMobileFromText(compliantDescription);
      defaultRulesApplied.push(`PC端描述已按规则重写，字符数 ${plainTextFromHtml(compliantDescription).length}/${EDIT_PAGE_RULES.pcDescriptionMinChars}`);
    }
  ...
  function validatePayload(payload) {
    ...
    if (hasForbiddenCommerceTerms(subjectText)) {
      risks.push('标题包含平台词或违规营销词，必须重新生成后再保存');
    }
  ...
  function applyVisibleTitleRule() {
    ...
    const title = buildCompliantProductTitle(original, source);
    const changed = setInputValue(input, title);
    return { changed, original, value: title, length: title.length };
  }
  ...
  async function selectPostageTemplate111() {
    ...
    const getSelectedText = () => {
      ...
      return selected ? elementText(selected) : '';
    };
  ...
  async function applyVisibleEditPageRules(options = {}) {
    ...
    const result = { at: nowIso(), manual: Boolean(options.manual), title, description, textSanitization, variation, postage, marketing };
    state.visibleEditRuleResult = result;
    if (!state.report) state.report = {};
    state.report.visibleEditRuleResult = result;
    log(`\u7f16\u8f91\u9875\u89c4\u5219\u5df2\u5e94\u7528\uff1a\u6807\u9898${title.changed ? '\u5df2\u6539' : '\u672a\u6539'}\uff0c\u63cf\u8ff0${description.changed ? '\u5df2\u6539' : '\u672a\u6539'}\uff0c\u5b57\u7b26\u6e05\u6d17${textSanitization.changed ? '\u5df2\u5904\u7406' : '\u65e0\u9700\u5904\u7406'}\uff0c\u8fd0\u8d39111${postage.changed ? '\u5df2\u9009' : '\u672a\u9009'}\uff0c\u8425\u9500\u56fe${marketing.changed ? '\u5df2\u89e6\u53d1' : '\u672a\u89e6\u53d1'}`, result);
    updateUi();
    return result;
  }

  function scheduleVisibleEditRulesAutoApply() {
    if (!isEditPage() || visibleEditRulesAutoApplied) return;
    visibleEditRulesAutoApplied = true;
